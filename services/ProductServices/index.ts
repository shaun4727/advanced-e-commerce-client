...
export const getAllProducts = async (
    ...
    try {
        const res = await fetch(
            `${process.env.NEXT_PUBLIC_BASE_API}/product?limit=${limit}&page=${query?.page}&${params}`,
            {
                next: {
                    tags: ['PRODUCT'],
                },
...
export const getMyShopProductsApi = async (page?: string, limit?: string) => {
    try {
        const res = await fetch(
            `${process.env.NEXT_PUBLIC_BASE_API}/product/my-shop-products?limit=${limit}&page=${page}`,
            {
                method: 'GET',
                headers: {
                    Authorization: (await cookies()).get(
